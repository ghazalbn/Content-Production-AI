...
def filter_by_keywords(news_data, keyword_weight_pairs):
    ...
    for index, row in news_data.iterrows():
        ...
        if matched_keywords and len(matched_keywords):
...
def news_details_page():
    ...
    matched_keywords = selected_news.get('matched_keywords', None)
    ...
    if len(tags_df) < 7:
        if st.button("تولید تگ"):
            ...
            if generated_tags:
                tag_ids = db_manager.insert_tags(generated_tags)  # Insert tags and get their IDs
                if tag_ids:
                    db_manager.insert_content_tags(news_id, tag_ids)  # Link tags to content
                    st.write(' ,'.join(generated_tags))
