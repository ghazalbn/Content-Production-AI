...
class ArticleGeneration:
    ...
    def gpt_generate_article(self, title, source, url, date, news_content, matched_keywords=None):
        keywords_str = ', '.join(matched_keywords) if matched_keywords and len(matched_keywords) and matched_keywords!=[''] else ""
        # return f"k: {keywords_str}"
        question = f"""
        Write an analytical article in Persian for Tokeniko.com based on the following news content. 
        Provide insights from an economist's perspective{' and include the following keywords (if applicable): ' + keywords_str if keywords_str else '.'}. 
        The article should be at least 1300 words. Generate an attractive and concise title for it. 
        The introduction has better to start with the topic. The article has better to contain tables and images if needed.
        ...
        **News Content**:
        ...
        logging.warning("Sending request to OpenAI for article generation")
